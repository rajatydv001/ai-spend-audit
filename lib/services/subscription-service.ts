  ...
  switch (event.type) {
    case "checkout.session.completed": {
      ...
      if (userId && plan) {
        ...
        await prisma.subscription.update({
          where: { id: (await prisma.user.findUnique({ where: { id: userId } }))?.subscriptionId! },
          data: {
            status: "ACTIVE",
            stripeSubscriptionId: session.subscription,
            stripePriceId: session.mode === "subscription" ? sub.items.data[0].price.id : null,
        ...
        await updateSubscriptionLimits(
          (await prisma.user.findUnique({ where: { id: userId } }))?.subscriptionId!,
          plan
        );
